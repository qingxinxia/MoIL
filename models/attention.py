...
class Seq_Transformer(nn.Module):
    ...
    def forward(self, forward_seq):  # input(B,Len,Dim6)
        x = self.patch_to_embedding(forward_seq)
        x = self.position(x)  # input(B,L,Dim128)
        b, n, _ = x.shape
        # c_tokens = repeat(self.c_token, '() n d -> b n d', b=b)  #add 1-dim token
        # x = torch.cat((c_tokens, x), dim=1)
        x = self.transformer(x)
        # c_t = self.to_c_token(x[:, 0])
        return x  # output(B,L,Dim128)
